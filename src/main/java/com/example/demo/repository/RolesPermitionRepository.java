...
public interface RolesPermitionRepository extends JpaRepository<RolesPermition, Long>{
	...
	@Query(value = "SELECT * FROM rolespermitions "
			+ "WHERE permition_id = ? AND role_id = ?;", nativeQuery = true)
	RolesPermition findRolesPermitionByPermitionANDRole(Long permitionId, Long roleId);
